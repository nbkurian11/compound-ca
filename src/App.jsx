...
function calculate(starting, monthly, annualRate, years) {
...
export default function App() {
  const [form, setForm] = useState({ starting: '10000', monthly: '500', rate: '7', years: '20' })
  const [account, setAccount] = useState('TFSA')
  const [submitted, setSubmitted] = useState({ form, account })
  const update = (key) => (value) => setForm((current) => ({ ...current, [key]: value }))

  const projection = useMemo(() => {
    const starting = number(submitted.form.starting)
    const monthly = number(submitted.form.monthly)
    const nominalRate = number(submitted.form.rate)
    const years = number(submitted.form.years)
    const taxableRate = Math.max(0, nominalRate - 2)
    const tfsa = calculate(starting, monthly, nominalRate, years)
    const taxable = calculate(starting, monthly, taxableRate, years)
    return {
      years, nominalRate, taxableRate, tfsa, taxable,
      selected: submitted.account === 'Taxable' ? taxable : tfsa,
      labels: Array.from({ length: Math.round(years) + 1 }, (_, year) => year === 0 ? 'Today' : `Year ${year}`),
    }
  }, [submitted])
  ...
  const selectedDescription = submitted.account === 'Taxable'
    ? `${projection.taxableRate.toFixed(1)}% effective annual return after tax drag`
    : `${projection.nominalRate.toFixed(1)}% annual return · ${submitted.account === 'TFSA' ? 'tax-free' : 'tax-deferred'}`
  ...
  return <main className="min-h-screen bg-[#fcfcfb] px-5 py-7 text-slate-900 sm:px-8 lg:px-12 lg:py-10">
    <div className="mx-auto max-w-7xl">
      ...
      <div className="grid border-y border-slate-200 lg:grid-cols-[330px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 py-8 lg:py-10 lg:pl-12">
          <div className="flex flex-wrap items-baseline justify-between gap-2"><div><h2 className="text-base font-semibold text-slate-950">Your projection</h2><p className="mt-1 text-sm text-slate-500">{selectedDescription}</p></div><p className="text-sm text-slate-500">{submitted.account}</p></div>
